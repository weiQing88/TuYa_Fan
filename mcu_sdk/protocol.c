...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_value_update(DPID_FAN_SPEED,当前风速); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_GEAR,当前档位); //枚举型数据上报;

    */
    mcu_dp_bool_update(DPID_SWITCH,SWITCH); //BOOL型数据上报;
    mcu_dp_value_update(DPID_FAN_SPEED,SPEED); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_GEAR,GEAR); //枚举型数据上报;
}
...
/*****************************************************************************
函数名称 : dp_download_gear_handle
功能描述 : 针对DPID_GEAR的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_gear_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char gear;

    gear = mcu_get_dp_download_enum(value,length);
    switch(gear) {
    case 0:
        FanClose();
        FanGear_1();
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_GEAR, gear);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    case DPID_GEAR:
        //档位处理函数
        ret = dp_download_gear_handle(value,length);
        break;
